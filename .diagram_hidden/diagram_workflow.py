...
def identify_diagram_features(img, ocr_capital_labels, easiest_anchor_points=[], expected_line_segments=[]):
    ...
    # Hough Transform for Lines (tuned for more generosity than last, but still reasonable)
    lines = cv2.HoughLinesP(edges, 1, np.pi / 180, 
                            threshold=80,      # Slightly lower threshold than 100
                            minLineLength=40,   # Slightly shorter lines allowed
                            maxLineGap=30)      # Slightly larger gaps allowed
    ...
    if ocr_capital_labels:
        for ocr_label_info in sorted_ocr_labels:
            ...
            expanded_bbox = [
                ...
                min(img.shape[1], int(bbox[2] + expand_x)),
                min(img.shape[0], int(bbox[3] + expand_y))
            ]

            min_dist_to_line_endpoint = float('inf')
            closest_line_endpoint = None

            # Iterate through all detected lines to find the closest endpoint outside the expanded bbox
            for line_seg in detected_raw_lines:
                p1, p2 = np.array(line_seg[0]), np.array(line_seg[1])

                # Check endpoint 1
                if not (expanded_bbox[0] <= p1[0] <= expanded_bbox[2] and expanded_bbox[1] <= p1[1] <= expanded_bbox[3]):
                    dist = np.linalg.norm(bbox_center - p1)
                    if dist < min_dist_to_line_endpoint:
                        min_dist_to_line_endpoint = dist
                        closest_line_endpoint = p1
                
                # Check endpoint 2
                if not (expanded_bbox[0] <= p2[0] <= expanded_bbox[2] and expanded_bbox[1] <= p2[1] <= expanded_bbox[3]):
                    dist = np.linalg.norm(bbox_center - p2)
                    if dist < min_dist_to_line_endpoint:
                        min_dist_to_line_endpoint = dist
                        closest_line_endpoint = p2
            
            # If a suitable line endpoint was found, use it
            if closest_line_endpoint is not None and min_dist_to_line_endpoint < float('inf'): # Ensure a point was actually found
                refined_points[label] = closest_line_endpoint
                print(f"Associated OCR label '{label}' with closest line endpoint {closest_line_endpoint.tolist()} (distance: {min_dist_to_line_endpoint:.2f}).")
                sys.stdout.flush()
            else:
                print(f"No suitable line endpoint found outside expanded bbox for OCR label '{label}' at {bbox_center.tolist()}. Initiating darkest pixel fallback.")
                sys.stdout.flush()
                ...
                fallback_search_radius = max(3 * bbox_height, 50, img_diagonal * 0.05) # A bit more generous for fallback search
...
def visualize_overlay_diagram(original_img_path, ideal_diagram_img_path, output_path="overlay_diagram.png"):
    """
    Creates a new image by overlaying the transparent ideal diagram on top of the original hand-drawn image.
    """
    print(f"--- Creating overlay diagram to {output_path} ---")
    sys.stdout.flush()
    try:
        # Load the original image
        original_img_pil = Image.open(original_img_path).convert("RGBA") # Ensure it has an alpha channel

        # Load the ideal diagram image (already RGBA from construct_ideal_diagram)
        ideal_img_pil = Image.open(ideal_diagram_img_path)

        # Ensure both images are the same size
        if original_img_pil.size != ideal_img_pil.size:
            print(f"Warning: Image sizes differ. Resizing ideal diagram from {ideal_img_pil.size} to {original_img_pil.size}.")
            sys.stdout.flush()
            ideal_img_pil = ideal_img_pil.resize(original_img_pil.size, Image.Resampling.LANCZOS)

        # Composite the ideal diagram onto the original image
        # alpha_composite works best when both images are RGBA
        overlay_img = Image.alpha_composite(original_img_pil, ideal_img_pil)

        overlay_img.save(output_path)
        print(f"Overlay diagram saved to {output_path}")
        sys.stdout.flush()
        return output_path
    except FileNotFoundError as e:
        print(f"Error: One or both images not found for overlay: {e}")
        sys.stdout.flush()
        return None
    except Exception as e:
        print(f"Error creating overlay diagram: {e}")
        sys.stdout.flush()
        return None
...
def main_workflow(image_path, question_text):
    ...
    if overlay_output_path:
        print(f"  Overlay Diagram Image saved at: {overlay_output_path}")
    sys.stdout.flush()
